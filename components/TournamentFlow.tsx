...
import { printTournamentReport, printBlankScoreSheet, printGironiTournament, printBeatTheBoxComplete, printBeatTheBoxBlank } from '../services/printService.ts';
...
const generateRoundRobinMatches = (pairs: [Player, Player][]): Omit<Match, 'id' | 'date' | 'winner' | 'sets'>[] => {
    ...
    if (pairs.length === 2) {
        ...
        matches.push({
            team1: [pairs[0][0].id, pairs[0][1].id],
            team2: [pairs[1][0].id, pairs[1][1].id],
        });
        return matches;
    }
    ...
    if (pairs.length === 3) {
        ...
        const allMatches = [
            { team1: [pairs[0][0].id, pairs[0][1].id], team2: [pairs[1][0].id, pairs[1][1].id] },
            { team1: [pairs[0][0].id, pairs[0][1].id], team2: [pairs[2][0].id, pairs[2][1].id] },
            { team1: [pairs[1][0].id, pairs[1][1].id], team2: [pairs[2][0].id, pairs[2][1].id] },
        ];
    ...
    if (pairs.length === 4) {
        // 4 pairs: use the classic round-robin schedule
        const schedule = [
            // Round 1
            { team1: [pairs[0][0].id, pairs[0][1].id], team2: [pairs[1][0].id, pairs[1][1].id] },
            { team1: [pairs[2][0].id, pairs[2][1].id], team2: [pairs[3][0].id, pairs[3][1].id] },
            // Round 2
            { team1: [pairs[0][0].id, pairs[0][1].id], team2: [pairs[2][0].id, pairs[2][1].id] },
            { team1: [pairs[1][0].id, pairs[1][1].id], team2: [pairs[3][0].id, pairs[3][1].id] },
            // Round 3
            { team1: [pairs[0][0].id, pairs[0][1].id], team2: [pairs[3][0].id, pairs[3][1].id] },
            { team1: [pairs[1][0].id, pairs[1][1].id], team2: [pairs[2][0].id, pairs[2][1].id] },
        ];
        return schedule;
    }
    ...
    if (pairs.length === 5) {
        ...
        const allMatches = [
            ...
            { team1: [pairs[0][0].id, pairs[0][1].id], team2: [pairs[2][0].id, pairs[2][1].id] },
            { team1: [pairs[0][0].id, pairs[0][1].id], team2: [pairs[3][0].id, pairs[3][1].id] },
            { team1: [pairs[0][0].id, pairs[0][1].id], team2: [pairs[4][0].id, pairs[4][1].id] },
            { team1: [pairs[1][0].id, pairs[1][1].id], team2: [pairs[2][0].id, pairs[2][1].id] },
            { team1: [pairs[1][0].id, pairs[1][1].id], team2: [pairs[3][0].id, pairs[3][1].id] },
            { team1: [pairs[1][0].id, pairs[1][1].id], team2: [pairs[4][0].id, pairs[4][1].id] },
            { team1: [pairs[2][0].id, pairs[2][1].id], team2: [pairs[3][0].id, pairs[3][1].id] },
            { team1: [pairs[2][0].id, pairs[2][1].id], team2: [pairs[4][0].id, pairs[4][1].id] },
            { team1: [pairs[3][0].id, pairs[3][1].id], team2: [pairs[4][0].id, pairs[4][1].id] },
        ];
    ...
    if (pairs.length >= 6) {
        ...
        for (let i = 0; i < pairs.length; i++) {
            for (let j = i + 1; j < pairs.length; j++) {
                allMatches.push({
                    team1: [pairs[i][0].id, pairs[i][1].id],
                    team2: [pairs[j][0].id, pairs[j][1].id],
                });
            }
        }
...
const TournamentFlow: React.FC<TournamentFlowProps> = ({ pairs, onFinish }) => {
    ...
    const existingTournamentNames = useMemo(() => Array.from(new Set(tournaments.map(t => t.name))), [tournaments]);
    ...
    const handleGironiFinalsConfirm = async () => {
        ...
        try {
            ...
            const tournamentData = {
                ...
                type: 'Gironi + Fase Finale' as const,
                date: tournamentDate,
                club: clubName,
                matchIds: [],
        ...
        try {
            ...
            const matchesToCreate = torneoLiberoMatches
                ...
                .map(match => {
                    ...
                    return {
                        team1: [pairs[team1PairIndex][0].id, pairs[team1PairIndex][1].id] as [string, string],
                        team2: [pairs[team2PairIndex][0].id, pairs[team2PairIndex][1].id] as [string, string],
            ...
            const tournamentData = {
                name: finalTournamentName,
                type: 'Torneo Libero' as const,
                date: tournamentDate,
                club: clubName,
                matchIds: [],
                status: 'scheduled' as const,
                giornataName: nomeTorneoLibero // Nome della giornata del torneo libero
            };
    ...
    const handleFinishScoring = async () => {
        ...
        try {
            ...
            alert("There was an error saving the tournament. Please try again.");
        } finally {
            setIsSubmitting(false);
        }
    };
    ...
    const handleSaveCalendar = async () => {
        ...
        try {
            ...
            alert("There was an error saving the calendar. Please try again.");
        } finally {
                ...
                <div className="space-y-6">
                    ...
                    <div className="flex gap-3 mt-6">
                        ...
                        <Button
                            ...
                            className="flex-1"
                        >
                            Conferma Gironi e Passa alle Semifinali
                        </Button>
                    </div>
                </div>
            </Card>
                ...
                <div className="space-y-6">
                                    ...
                                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                        ...
                                        <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                                            {standing.map((entry: any, idx: number) => {
                                                // Primi di ogni girone qualificati + migliori seconde
                                                const isQualified = idx === 0 || (idx === 1 && gironeIdx < numGironi);
                    ...
                    <div className="mt-6 p-3 bg-green-50 dark:bg-green-900/30 rounded-lg border border-green-200 dark:border-green-800">
                        ...
                        <ul className="text-sm text-green-800 dark:text-green-300 space-y-1">
                            ...
                            {gironiStandings.map((standing, idx) => {
                                if (standing[1]) {
                                    const numSecondeNeeded = 4 - Math.min(gironi.length, 4);
                                    if (idx < numSecondeNeeded) {
                                        const second = standing[1];
                                        const gironeName = String.fromCharCode(65 + idx);
                                        return (
                                            <li key={`second-${idx}`}>
                                                <strong>2° Girone {gironeName}:</strong> {second.pair[0].name} {second.pair[0].surname} & {second.pair[1].name} {second.pair[1].surname}
                                            </li>
                                        );
                                    }
                                }
    ...
    if (step === 'setup') {
        return (
            <Card title={`Tournament Setup: ${getFormatDisplayName(selectedFormat!)}`}>
                <div className="space-y-4">
                     <div>
                        <label className="block text-sm font-medium text-gray-500 dark:text-gray-400">Tournament</label>
                        <div className="mt-1 flex rounded-md shadow-sm">
                            <button onClick={() => { console.log('🆕 New button clicked'); setIsCreatingNew(true); setClubName(''); setSelectedTournamentName(''); setSavedTournamentType(''); }} className={`px-4 py-2 rounded-l-md text-sm font-medium focus:outline-none ${isCreatingNew ? 'bg-sky-600 text-white' : 'bg-gray-200 dark:bg-gray-600'}`}>New</button>
                            <button onClick={() => { console.log('📁 Existing button clicked'); setIsCreatingNew(false); }} disabled={existingTournamentNames.length === 0} className={`px-4 py-2 rounded-r-md text-sm font-medium focus:outline-none ${!isCreatingNew ? 'bg-sky-600 text-white' : 'bg-gray-200 dark:bg-gray-600'} disabled:opacity-50`}>Existing</button>
                        </div>
                    </div>

                    {isCreatingNew ? (
                        <div>
                            <label htmlFor="tournamentName" className="block text-sm font-medium text-gray-500 dark:text-gray-400">New Tournament Name</label>
                            <input
                                type="text"
                                id="tournamentName"
                        ...
                        <div>
                            <label htmlFor="selectTournament" className="block text-sm font-medium text-gray-500 dark:text-gray-400">Select Existing Tournament</label>
                             <select
                                id="selectTournament"
                    ...
                    <div>
                        <label htmlFor="tournamentDate" className="block text-sm font-medium text-gray-500 dark:text-gray-400">Date for this Match Day</label>
                        <input
                            type="date"
                            id="tournamentDate"
                    ...
                    <div>
                        <label htmlFor="clubName" className="block text-sm font-medium text-gray-500 dark:text-gray-400">Club Name</label>
                        <input
                            type="text"
                            id="clubName"
                            value={clubName}
                            onChange={e => setClubName(e.target.value)}
                            placeholder="e.g., Padel Club Milano"
                            className="mt-1 block w-full bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-sky-500 focus:border-sky-500 sm:text-sm disabled:bg-gray-200 dark:disabled:bg-gray-600"
                ...
                <Card title={
                    ...
                    <div className="flex gap-3 mt-6">
                        ...
                        <Button 
                            ...
                            variant="secondary"
                            className="flex-1"
                            disabled={!torneoLiberoMatches.every(match => match.team1 && match.team2) || isSubmitting}
                        >
    ...
    if (step === 'scoring') {
        ...
        return (
            <>
                <Card title={
                     <div className="flex justify-between items-center">
                        <span>Enter Match Scores - {getFormatDisplayName(selectedFormat!)}</span>
                        <Button onClick={handlePrintBlank} variant="ghost" size="sm">
                            <PrintIcon /> Stampa Tabellone Vuoto
                        </Button>
                    </div>
                    ...
                    <div className="flex gap-3 mt-6">
                        ...
                        >
                            {isSavingCalendar ? "Saving..." : "Salva Calendario"}
                        </Button>
                        <Button 
                        ...
                        <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            <thead className="text-xs text-gray-700 dark:text-gray-300 uppercase bg-gray-50 dark:bg-gray-700">
                                <tr>
                                    ...
                                    <th scope="col" className="px-4 py-3">Games Won</th>
                                    <th scope="col" className="px-4 py-3">Games Lost</th>
                    ...
                    <div className="flex justify-between mt-6">
                        <Button onClick={() => {
                            ...
                            if (createdTournament.type === TournamentType.BeatTheBox) {
                                ...
                                const boxStandings = calculateAllBoxStandings(boxes);
                                const semifinalMatches: Match[] = [];
                                const finalMatches: Match[] = [];
                                const individualStandings: { player: Player; eloChange: number; rank: number }[] = [];
                        ...
                        }} variant="secondary">Stampa Report</Button>
                        <Button onClick={handleFinishAndClose}>Chiudi</Button>
                    </div>
                </Card>
