...
const MatchesPage: React.FC<MatchesPageProps> = ({ tournamentToOpen, setTournamentToOpen }) => {
    const { players, matches, tournaments, eloHistory, addMatch, deleteMatch, getPlayerById, deleteTournament, updateTournamentMatches, loading } = usePadelStore();
    ...
    useEffect(() => {
        if (tournamentToOpen && tournaments.length > 0) {
            ...
            if (tournament) {
                ...
                // BEAT THE BOX: Check if box matches are already completed
                if (tournament.type === TournamentType.BeatTheBox && tournament.status === 'scheduled') {
                    const tournamentMatches = matches.filter(m => m.tournamentId === tournament.id);
                    const boxMatchCount = Math.floor(tournamentMatches.length / 3) * 3; // Box matches are multiples of 3
                    const boxMatches = tournamentMatches.slice(0, boxMatchCount);
                    const allBoxMatchesCompleted = boxMatches.length > 0 && boxMatches.every(m => m.winner && m.sets.length > 0);
                    
                    if (allBoxMatchesCompleted) {
                        console.log(`📦 Beat the Box: Box matches already completed, skipping to finals...`);
                        
                        // Calculate standings and proceed to finals
                        const numBoxes = boxMatchCount / 3;
                        setBeatBoxNumBoxes(numBoxes);
                        
                        // Create boxesData
                        const boxesData = [];
                        for (let i = 0; i < numBoxes; i++) {
                            const matches = boxMatches.slice(i * 3, (i + 1) * 3);
                        ...
                        // Calculate standings
                        const standings = calculateAllBoxStandings(boxMatches, boxesData);
                        setBeatBoxStandings(standings);
                        
                        // Check if finals/semifinals matches exist
                        const finalsMatches = tournamentMatches.slice(boxMatchCount);
                        
                        if (finalsMatches.length === 0) {
                            // No finals yet, show standings modal
                            setShowBeatBoxStandingsModal(true);
                            setFinalsFlowTournament(tournament);
                        } else {
                            // Finals exist, check if they're complete
                            const allFinalsCompleted = finalsMatches.every(m => m.winner && m.sets.length > 0);
                            
                            if (allFinalsCompleted) {
                                // Everything is done, just open for viewing
                                setEditingTournament(tournament);
                            } else {
                                // Finals exist but not complete, show them
                                setFinalsFlowTournament(tournament);
                                if (numBoxes >= 4 && finalsMatches.length >= 2) {
                                    // Has semifinals
                                    const semiMatches = finalsMatches.slice(0, 2);
                                    const finalMatches = finalsMatches.slice(2);
                                    const allSemisCompleted = semiMatches.every(m => m.winner && m.sets.length > 0);
                                    
                                    if (allSemisCompleted && finalMatches.length > 0) {
                                        // Semis done, show finals
                        ...
                        return; // Don't open normal editing modal
                    }
                }
                
                // Open the editing modal for normal case
                setEditingTournament(tournament);
            }
    ...
    const generateFinalsMatches = (top4Standings: TournamentStandingEntry[]): Match[] => {
        ...
        return [
            ...
            {
                id: generateMatchId(),
                date: editingTournament?.date || new Date().toISOString(),
    ...
    const handleEditScoresSubmit = async (e: React.FormEvent) => {
        ...
        const matchUpdates = Object.keys(editScores).map((matchId) => ({
            matchId,
            sets: editScores[matchId],
        }));

        // Pareggi sono permessi nei tornei round robin come TorneOtto 30'
        
        setIsSubmitting(true);
        try {
            await updateTournamentMatches(matchUpdates);
            
            // Check if this is a Round Robin + Finali tournament
            ...
            if (editingTournament.status === 'scheduled' && isBeatTheBox && !isInBeatBoxFinalsPhase && !isInBeatBoxSemifinalsPhase) {
                console.log(`📦 Beat the Box: Box phase completed, calculating standings...`);
                
                // Save tournament reference
                setFinalsFlowTournament(editingTournament);
                
                // Get all matches with updated scores
                const tournamentMatches = matches.filter(m => m.tournamentId === editingTournament.id).map(match => {
                ...
                // Calcola numero box: ogni box ha 3 partite, ogni partita ha 4 giocatori unici
                // Totale giocatori = numBoxes * 4, Totale partite = numBoxes * 3
                const numBoxes = tournamentMatches.length / 3;
                setBeatBoxNumBoxes(numBoxes);
                
                // Crea boxesData per il calcolo classifiche
                const boxesData = [];
                for (let i = 0; i < numBoxes; i++) {
                    const boxMatches = tournamentMatches.slice(i * 3, (i + 1) * 3);
                    const allPlayers = new Set<string>();
                    boxMatches.forEach(m => {
                        m.team1.forEach(id => allPlayers.add(id));
                        m.team2.forEach(id => allPlayers.add(id));
                    });
                    const boxPlayers = Array.from(allPlayers).map(id => getPlayerById(id)!).filter(Boolean);
                    boxesData.push({
                        boxNumber: i + 1,
                        players: boxPlayers,
                        matches: boxMatches,
                ...
                const standings = calculateAllBoxStandings(tournamentMatches, boxesData);
                setBeatBoxStandings(standings);
            ...
            // If tournament is scheduled and is Round Robin + Finali, calculate standings and prepare finals
            if (editingTournament.status === 'scheduled' && isRoundRobinFinali && !isInFinalsPhase) {
                console.log(`🎯 Round Robin phase completed, calculating standings...`);
                
                // Save tournament reference for finals flow
                setFinalsFlowTournament(editingTournament);
            ...
            // If tournament is scheduled and NOT Round Robin + Finali, complete it normally
            if (editingTournament.status === 'scheduled') {
    ...
    const { individualMatches, tournamentsByName, sortedTournamentNames } = useMemo(() => {
        ...
        tournaments.forEach(t => {
            if (!tournamentsByName[t.name]) {
                tournamentsByName[t.name] = [];
            }
            tournamentsByName[t.name].push(t);
        });
    ...
    return (
        <div className="space-y-6">
            <Card title="Inserisci Risultato Singolo">
            ...
            <Card title="Elenco Partite e Giornate">
                                                    ...
                                                    <div key={day.id} className="bg-gray-50 dark:bg-gray-800/50 rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700">
                                                        <div className="p-3 flex justify-between items-center cursor-pointer hover:bg-gray-200/50 dark:hover:bg-gray-700/50" onClick={() => toggleExpand(`day_${day.id}`)}>
                                                            <div className="flex items-center">
                                                                ...
                                                                <div>
                                                                    <p className="font-semibold text-gray-900 dark:text-white">{day.type} @ {day.club}</p>
                                                            ...
                                                            <div className="flex items-center space-x-2" onClick={e => e.stopPropagation()}>
                                                                <Button variant="secondary" size="sm" onClick={() => setEditingTournament(day)} className="!p-2"><PencilIcon /></Button>
                                                                <Button variant="danger" size="sm" onClick={() => handleDeleteTournament(day.id)} className="!p-2"><TrashIcon /></Button>
                                                                <Button variant="secondary" size="sm" onClick={() => handlePrintTournament(day)} className="!p-2"><PrintIcon /></Button>
                                                            </div>
                                                        </div>
                                                        {isDayExpanded && (
                                                                ...
                                                                {matches.filter(m => m.tournamentId === day.id).map((match, index) => {
                                                                    ...
                                                                    return (
                                                                        <div key={match.id} className="flex items-center justify-between text-sm">
                                                                            ...
                                                                            <span className={`mx-4 font-bold ${day.status === 'scheduled' ? 'text-gray-400' : 'text-sky-500'}`}>
                                                                                {day.status === 'scheduled' ? '0-0' : match.sets.map(s => `${s.team1}-${s.team2}`).join(' ')}
                                                                            </span>
                            ...
                            return (
                                <div key={match.id} className="bg-gray-100 dark:bg-gray-800 p-4 rounded-lg shadow">
                                    <div className="flex justify-between items-center mb-2">
                                        <p className="text-sm font-semibold text-gray-700 dark:text-gray-300">Friendly Match</p>
                                    ...
                                    <div className="text-center text-xs text-gray-500 dark:text-gray-400 mt-2">
                                        ELO Change: <span className={`font-semibold ${eloChange > 0 ? 'text-green-500' : 'text-red-500'}`}>
                                            {eloChange !== 0 ? `±${eloChange.toFixed(2)}` : 'N/A'}
                                        </span>
                                        <span className="italic ml-2">(Note: ELO change shown for historical matches only)</span>
                                    </div>
                                </div>
                            );
                        })}
            ...
            <Modal 
                isOpen={!!editingTournament && !isInFinalsPhase && !showFinalsStandingsModal && !showBeatBoxStandingsModal && !isInBeatBoxFinalsPhase && !isInBeatBoxSemifinalsPhase} 
                onClose={() => {
                    // Only reset if we're not in the finals flow
                    if (!showFinalsStandingsModal && !isInFinalsPhase && !showBeatBoxStandingsModal && !isInBeatBoxFinalsPhase && !isInBeatBoxSemifinalsPhase) {
                        setEditingTournament(null);
                        setIsInFinalsPhase(false);
                        setShowFinalsStandingsModal(false);
                ...
                title={editingTournament?.status === 'scheduled' ? "Enter Match Scores" : "Edit Match Scores"}
            >
            ...
            >
                <div className="space-y-4">
                    ...
                    <div className="flex gap-3 pt-4">
                        ...
                        <Button 
                            onClick={async () => {
                                // Genera finali basate sui risultati delle semifinali
